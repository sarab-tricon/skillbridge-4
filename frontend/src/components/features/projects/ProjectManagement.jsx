...
const ProjectManagement = () => {
        ...
        try {
            ...
            const employeesWithUtil = await Promise.all(
                employeesRes.data.map(async (emp) => {
                    try {
                        const utilRes = await allocationsApi.getEmployeeUtilization(emp.id);
                        return {
                            ...emp,
    ...
    const handleEndProject = async (projectId) => {
        ...
        try {
            // Assuming the endpoint expects just the status field update. 
            // The original logic used logic similar to updateProject but targeted specific endpoint if custom.
            // Based on original code: api.put(`/projects/${projectId}/status`, { status: 'COMPLETED' });
            // I need to add this to projectsApi or use updateProject if it supports it.
            // I'll assume I need to add updateStatus to projectsApi or just use updateProject if backend supports generic update.
            // I'll assume I need to add updateStatus to projectsApi momentarily.
            // For now, I'll use a direct call pattern via the api object if I exposed it, but I didn't.
            // I'll add it to projectsApi in a separate step or just include it here if I modify projects.js.
            // Let's modify projects.js first to include updateProjectStatus.
            // Actually, I can't modify projects.js mid-tool-call. 
            // I will implement it assuming I will add it to projects.js immediately after.
            await projectsApi.updateStatus(projectId, { status: 'COMPLETED' });
            // Wait, original was `/projects/${projectId}/status`. Generic update might not work if backend is specific.
            // I will check projects.js content again. It has updateProject. 
            // If the backend has a specific status endpoint, I should use that. 
            // Let's guess the backend is RESTful. 
            // To be safe, I'll update projects.js to include updateProjectStatus.
